...
def populate_demo_data(db):
    """Inserts the 5 demo meetings into the provided database session."""
    # -------------------------------------------------------------
    ...
    db.commit()
    print("Database successfully seeded with 5 comprehensive demo meetings, transcripts, summaries, topics, and action items!")

def seed_if_empty():
    """Automatically seed 5 demo meetings if no demo meetings exist."""
    db = SessionLocal()
    try:
        existing_demo = db.query(Meeting).filter(Meeting.user_id.is_(None)).first()
        if not existing_demo:
            print("No demo meetings found in database. Auto-seeding 5 demo meetings...")
            populate_demo_data(db)
        else:
            print("Demo meetings already exist in database.")
    except Exception as e:
        db.rollback()
        print(f"Auto-seed skipped or encountered error: {e}")
    finally:
        db.close()

def seed_database():
    """Force re-creates all tables and seeds the database cleanly."""
